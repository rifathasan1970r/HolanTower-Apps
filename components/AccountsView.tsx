  ...
  useEffect(() => {
    if (editingMonth && accounts.length > 0) {
      ...
      if (account) {
        ...
        setIsNoteEnabled(!!account.income.note);
      }
    }
                                  ...
                                  <div className="relative z-10 space-y-3">
                                    <div className="flex items-center justify-between">
                                      ...
                                      {isAuthorized && (
                                        <button
                                          onClick={() => {
                                            if (isNoteEnabled && isAuthorized) {
                                              setEditData({ ...editData, income: { ...editData.income, note: '' } });
                                            }
                                            setIsNoteEnabled(!isNoteEnabled);
                                          }}
                                          className={`relative inline-flex h-5 w-9 items-center rounded-full transition-colors focus:outline-none ${isNoteEnabled ? 'bg-emerald-500' : 'bg-slate-300'}`}
                                        >
                                          <span className={`inline-block h-3.5 w-3.5 transform rounded-full bg-white transition-transform ${isNoteEnabled ? 'translate-x-4' : 'translate-x-1'}`} />
                                        </button>
                                      )}
                                    </div>
